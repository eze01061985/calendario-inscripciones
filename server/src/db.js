...
export async function migrate(pool) {
  // This schema is idempotent and is applied on every boot, including Railway deployments.
...
export async function listMonth(pool, year, month) {
  ...
  const [rows] = await pool.execute('SELECT id, fecha, nombre, creado_en, actualizado_en FROM inscripciones WHERE fecha >= ? AND fecha < ? ORDER BY fecha', [start, next]);
  return rows;
}
...
export async function createBooking(pool, fecha, nombre) {
  try {
    ...
    return { id: result.insertId, fecha, nombre };
  } catch (error) {
...
export async function updateBooking(pool, id, fecha, nombre) {
  try {
    const [result] = await pool.execute('UPDATE inscripciones SET fecha = ?, nombre = ? WHERE id = ?', [fecha, nombre, id]);
