...
function Calendar({ month, bookings, onSelect, onOccupied, admin }) {
  ...
  return <div className="calendar" role="grid" aria-label={`Calendario de ${monthTitle(month)}`}>
    ...
    {cells.map((day, i) => {
      ...
      const date = isoDate(year, index, day), booking = byDate.get(date), past = !admin && date < localToday();
      return <button key={date} type="button" role="gridcell" className={`day ${booking ? 'occupied' : ''} ${past ? 'past' : ''}`} disabled={past} onClick={() => booking ? onOccupied?.(booking) : onSelect(date)} aria-label={`${longDate(date)}: ${booking ? `anotado ${booking.nombre}` : past ? 'fecha pasada' : 'disponible, tocar para anotarse'}`}>
        <span className="day-number">{day}</span>{booking && <span className="day-name">{booking.nombre}</span>}
      </button>;
    })}
  </div>;
}
...
function MonthNavigation({ month, setMonth }) {
  ...
  const now = new Date();
  return <div className="month-nav"><button type="button" className="nav-arrow" onClick={() => move(-1)} aria-label="Mes anterior">‹</button><div className="month-heading"><h1 style={{ textTransform: 'none' }}>{monthTitle(month)}</h1><button type="button" className="today-link" onClick={() => setMonth(new Date(now.getFullYear(), now.getMonth(), 1))}>Volver a hoy</button></div><button type="button" className="nav-arrow" onClick={() => move(1)} aria-label="Mes siguiente">›</button></div>;
}
...
function PublicPage() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      setBookings(current);
      if (current.some(item => item.fecha === selected)) throw Object.assign(new Error('Ese día acaba de ser ocupado por otra persona. Elegí otro día disponible.'), { status: 409 });
...
function AdminPage() {
  ...
  const [bookings, setBookings] = useState([]), [all, setAll] = useState([]), [editing, setEditing] = useState(null), [name, setName] = useState(''), [date, setDate] = useState(''), [error, setError] = useState(''), [notice, setNotice] = useState(''), [busy, setBusy] = useState(false);
  const refresh = useCallback(async () => {
    try {
      const [monthData, allData] = await Promise.all([api(`/api/inscripciones?year=${month.getFullYear()}&month=${month.getMonth() + 1}`), api('/api/admin/inscripciones')]);
      setBookings(monthData); setAll(allData); setError('');
  ...
  return <main className="shell"><header className="topline"><a href="/">← Calendario público</a><button className="text-button" onClick={logout}>Cerrar sesión</button></header><div className="admin-heading"><span className="eyebrow">Administración</span><button className="primary small" onClick={() => open(null)}>+ Agregar</button></div><MonthNavigation month={month} setMonth={setMonth}/>
    {notice && <div className="notice success" role="status">{notice}</div>}{error && !editing && <div className="notice error" role="alert">{error}</div>}
    <Calendar month={month} bookings={bookings} admin onSelect={d => open(null, d)} onOccupied={open}/>
