...
- Evita la doble reserva incluso con peticiones simultáneas: `UNIQUE(fecha)` en MySQL es la autoridad final y la API traduce duplicados en HTTP 409.
...
server/migrations/      Esquema reproducible
...
Un conflicto de fecha responde `409`. Los usuarios públicos no disponen de PUT, PATCH ni DELETE para inscripciones.
...
4. `npm run build && npm start` abre el servidor en `http://localhost:3000`. Al arrancar aplica `server/migrations/001_initial.sql` de forma idempotente.
